...
```{r}
ggplot(one_stage, aes(x = factor(""), fill=color)) + geom_histogram(position="dodge") + facet_wrap(~ element) + scale_fill_manual(values = levels(one_stage$color), guide = "none") + xlab("Colores")
```
...
ggplot(my_data, aes(x = as.factor(value), fill = variable)) + geom_histogram(position = 'dodge', width = .8) + xlab('Qué tan forzado le pareció')
```
